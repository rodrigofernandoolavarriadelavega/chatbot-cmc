...
Diseño (mismos principios que agenda_ticker.py):
  1. Cursor persistido en `system_state` (`email_ticker_last_uid`) = mayor
     UID de IMAP ya procesado (estable per buzón, ver UIDVALIDITY). Arranque
     en frío: se siembra con el UID más alto existente (UIDNEXT-1) SIN leer
     los ~17.000 correos históricos del buzón.
  ...
  4. El correo NO trae el `id_cita` de Medilink. Se cruza por
     paciente + profesional + fecha + hora en DOS niveles: (a) primero
     contra la tabla local `agenda_ticker` (espejo de `/citas`, gratis, sin
     tocar Medilink), y (b) si no hay match, contra Medilink EN VIVO —
     `resolver_cruces_pendientes()` hace UNA consulta base paginada por
     fecha DISTINTA del lote (nunca una por correo ni un fan-out por
     profesional — mismo patrón ya usado en agenda_ticker.poll_agenda_ticker
     y sync_atenciones_sin_cerrar). Validado empíricamente 2026-07-14 contra
     26 correos reales (agendada/anulada/reagendada): el nivel (a) solo
     encuentra match si agenda_ticker ya venía corriendo antes del correo; el
     nivel (b), con paginación completa, cruzó 26/26. Si tras ambos niveles
     hay 0 o más de 1 candidato, se marca `no_cruzado` — nunca se adivina.
...
def _match_en_citas_medilink(paciente: str, profesional: str, hora: str, citas: list[dict]) -> int | None:
    """Mismo criterio de cruce que `cruzar_con_agenda_ticker` pero sobre una
    lista de citas crudas de Medilink (`nombre_paciente`/`nombre_profesional`/
    `hora_inicio`/`id`, ya filtradas por fecha). None si 0 o >1 candidatos."""
    candidatos = []
    for c in citas:
        if (c.get("hora_inicio") or "")[:5] != hora:
            continue
        if not _nombres_coinciden(paciente, c.get("nombre_paciente", "")):
            continue
        if not _nombres_coinciden(profesional, c.get("nombre_profesional", "")):
            continue
        candidatos.append(c.get("id"))
    if len(candidatos) == 1:
        return candidatos[0]
    return None


# Tope defensivo: cuántas fechas DISTINTAS se consultan a Medilink en vivo por
# pasada del poller (respaldo, solo para correos que NO cruzaron contra el
# caché local). En operación normal (poll cada 60s) esto es 0-2 fechas por
# pasada — el tope solo protege contra una ráfaga anómala de correos
# atrasados en un mismo ciclo. Cada fecha es UNA consulta base paginada
# (mismo patrón que agenda_ticker.poll_agenda_ticker), nunca un fan-out.
_MEDILINK_FALLBACK_MAX_FECHAS = 8
_MEDILINK_FALLBACK_MAX_PAGINAS = 8


async def _fetch_citas_medilink_por_fecha(client, headers: dict, fecha_iso: str, max_paginas: int = _MEDILINK_FALLBACK_MAX_PAGINAS) -> list[dict]:
    """UNA consulta base (`id_sucursal` + `fecha` exacta) paginada
    secuencialmente — validado empíricamente 2026-07-14 contra 26 correos
    reales: sin paginar se pierden citas en fechas con más de 50 citas
    (Medilink pagina de a 50) y el cruce falla en falso. Con paginación
    completa: 26/26 cruzados correctamente."""
    import json as _json
    import httpx
    from config import MEDILINK_BASE_URL, MEDILINK_SUCURSAL

    q = {"id_sucursal": {"eq": int(MEDILINK_SUCURSAL)}, "fecha": {"eq": fecha_iso}}
    url = f"{MEDILINK_BASE_URL}/citas"
    params = {"q": _json.dumps(q, separators=(",", ":"))}
    out: list[dict] = []
    paginas = 0
    try:
        while paginas < max_paginas:
            ...
            if r.status_code != 200:
                log.warning("_fetch_citas_medilink_por_fecha: HTTP %d fecha=%s", r.status_code, fecha_iso)
                break
            j = r.json()
        ...
        log.warning("_fetch_citas_medilink_por_fecha: fallo de red fecha=%s: %s", fecha_iso, e)
    return out


async def resolver_cruces_pendientes(registros: list[dict]) -> None:
    """Segunda pasada, en sitio: para los registros que NO cruzaron contra
    el caché local (`agenda_ticker`), consulta Medilink en vivo — UNA
    consulta paginada por cada fecha DISTINTA presente en el lote (no una
    por correo). Modifica `registros` in-place (`id_cita`/`cruce_status`)."""
    from medilink import HEADERS, _get_shared_client

    pendientes = [r for r in registros if r["cruce_status"] == "no_cruzado"]
    if not pendientes:
        return
    fechas = sorted({r["parsed"]["fecha_cita"] for r in pendientes})
    if len(fechas) > _MEDILINK_FALLBACK_MAX_FECHAS:
        log.warning(
            "resolver_cruces_pendientes: %d fechas distintas en un solo lote (> %d) — "
            "se consultan solo las primeras %d, el resto queda no_cruzado",
            len(fechas), _MEDILINK_FALLBACK_MAX_FECHAS, _MEDILINK_FALLBACK_MAX_FECHAS,
        )
        fechas = fechas[:_MEDILINK_FALLBACK_MAX_FECHAS]

    client = _get_shared_client()
    citas_por_fecha: dict[str, list[dict]] = {}
    for f in fechas:
        try:
            citas_por_fecha[f] = await _fetch_citas_medilink_por_fecha(client, HEADERS, f)
        except Exception as e:
            log.warning("resolver_cruces_pendientes: fallo consultando fecha=%s: %s", f, e)
            citas_por_fecha[f] = []

    for r in pendientes:
        f = r["parsed"]["fecha_cita"]
        cands = citas_por_fecha.get(f)
        if cands is None:
            continue  # fecha fuera del tope, se deja no_cruzado
        id_cita = _match_en_citas_medilink(
            r["parsed"]["paciente_nombre"], r["parsed"]["profesional_nombre"],
            r["parsed"]["hora_cita"], cands,
        )
        if id_cita is not None:
            r["id_cita"] = id_cita
            r["cruce_status"] = "cruzado"
...
async def poll_email_ticker() -> dict:
    ...
    try:
        ...
        # Pasada 1: filtrar remitente/asunto, parsear y cruzar contra el
        # caché local (agenda_ticker) — gratis, sin tocar Medilink.
        registros = []
        for m in raw_emails:
            try:
                ...
                registros.append({
                    "m": m, "parsed": parsed, "dt_local": dt_local,
                    "id_cita": id_cita, "cruce_status": cruce_status,
                })
            except Exception as e:
                log.error("poll_email_ticker: fallo parseando UID %s: %s", m.get("uid"), e)
                continue

        # Pasada 2: respaldo Medilink en vivo SOLO para lo que no cruzó local
        # — una consulta paginada por fecha DISTINTA del lote (ver
        # resolver_cruces_pendientes), nunca una por correo.
        try:
            await resolver_cruces_pendientes(registros)
        except Exception as e:
            log.warning("poll_email_ticker: resolver_cruces_pendientes falló, se sigue con lo cruzado local: %s", e)

        # Pasada 3: persistir + alimentar agenda_ticker.
        procesados = 0
        for reg in registros:
            try:
                m, parsed, dt_local = reg["m"], reg["parsed"], reg["dt_local"]
                id_cita, cruce_status = reg["id_cita"], reg["cruce_status"]
                ...
                log.error("poll_email_ticker: fallo persistiendo UID %s: %s", reg.get("m", {}).get("uid"), e)
                continue
